...
class FFNNetwork:
    ...
    def __init__(self,
        ...
        self.zs          = None
        self.gd_method_w = list()
        self.gd_method_b = list()
        ...
        for e in range(epochs):
            for _ in range(batches):
                ...
                for (W,b),(dW,db) in zip(self.layers,self.layers_grad):
                    ...
                    W -= self.gd_method_w[i].update_change(dW,W)
                    b -= self.gd_method_b[i].update_change(db,b)

                    i += 1
            
            ## Resetting instances variables for the SGD-methods
            for gd in self.gd_method_w:
                gd.reset()
            for gd in self.gd_method_b:
                gd.reset()
...
class LinearRegressor:
    ...
    def linear_predict(self, X, target, predict_type='OLS',lmbda=0.):
        """
        Method for calculating the beta's in a linear regression analysis, mainly for comparison purposes.
        Implemented for OLS, Ridge and Lasso - methods. Lasso method depends on sklearn.linear_model.Lasso
        """
    ...
    def predict(self,beta,X,target):
        """
        General method for returning the cost function error for general regression features beta
        """
